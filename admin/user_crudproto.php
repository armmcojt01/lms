    ...
    $mailer_path = __DIR__. '/../inc/mailerconfigadmin.php';
    if (file_exists($mailer_path)){
    require_once $mailer_path;
    } else{ 
    die ("Mailer configuration file not found. Please contact the IMS.");
    ...
    $act = $_GET['act'] ?? '';
    //Email for pending request admin confirmation
     if (isset($_GET['act']) && $_GET['act'] === 'confirm' && isset($_GET['id'])) {
        ...
        if ($current) {
            ...
            if ($current['status'] !== 'confirmed') {
                ...
                sendAdminConfirmationEmail($current['email'], $current['fname']);
            }
