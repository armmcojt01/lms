...
if ($act === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Insert user
        $stmt = $pdo->prepare(
            "INSERT INTO users (username,password,fname,lname,email,role,status,created_at,departments)
             VALUES (?,?,?,?,?,?,'confirmed',NOW(),?)"
        );
        $stmt->execute([$username, $hash, $fname, $lname, $email, $role, $departmentsJson]);
...
//department//////////////////////////////////////////////////////////////////////////////////
$department = $pdo->query("SELECT * FROM departments")->fetchAll(PDO::FETCH_ASSOC);
